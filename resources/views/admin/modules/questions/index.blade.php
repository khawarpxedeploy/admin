                        ...
                        <div class="card-header text-right">
                            <a href="{{ route('admin:question.create') }}">
                                <button class="btn btn-sm btn-icon btn-success" type="button">
                                    ...
                                    <span class="btn-inner--text"> {{ __('Add New Question') }}</span>
                                </button>
                            </a>
                        </div>
                                ...
                                <tbody>
                                    ...
                                    @if (isset($questions) && !empty($questions))
                                        @foreach ($questions as $key => $value)
                                            @php
                                                $i = $i + 1;
                                                $edit_url = url('/admin/question/edit/' . $value->id);
                                            @endphp
                                            <tr>
                                                <td>
                                                    ...
                                                    <div class="dropdown">
                                                        ...
                                                        <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow"
                                                            ...
                                                            <a class="dropdown-item"
                                                                href="{{ route('admin:question.destroy', [$value->id ?? '']) }}">Delete</a>
                                                        </div>
                                                    </div>
                                                </td>
                                            </tr>
    ...
    @push('js')
        ...
        <script type="text/javascript">
            $('.status-change').on('click', function() {
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{ route("admin:question.status") }}',
                    data: {
                        'status': status,
                        'id': id,
                        '_token': "{{ csrf_token() }}"
